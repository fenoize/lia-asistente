...
export async function buildBriefContext(
  ...
  const [profileRes, tasksRes, meetingsRes, remindersRes, projectsRes, contactsRes] = await Promise.all([
    supabase
      .from("profiles")
      .select("name, role, goals, timezone, assistant_name, assistant_gender")
      .maybeSingle(),
    supabase
      .from("tasks")
      .select("id, title, priority, start_date, due_date, status, project_id")
      .order("due_date", { ascending: true, nullsFirst: false })
      .limit(80),
    supabase
      .from("meetings")
      ...
      .gte("datetime", todayRange.startIso)
      .lt("datetime", tomorrowRange.startIso)
      .order("datetime", { ascending: true }),
    supabase
      .from("reminders")
    ...
    supabase.from("contacts").select("id, name").limit(50),
  ]);
  ...
  const gender = (
    (profile as any).assistant_gender === "masculine" ? "masculine" : "feminine"
  ) as "feminine" | "masculine";
  ...
  return {
    ...
    briefClientCount: briefClientNames.length,
    briefClientNames: briefClientNames.join(", "),
    todayMeetingCount: meetings.length,
    todayMeetings: bullets(meetings.map(fmtMeeting), "(ninguna)"),
    activeReminderCount: reminders.length,
    activeReminders: bullets(
